...
function ReportsPage() {
  ...
  const getTherapistSpecificAnalytics = () => {
    ...
    const completedSessions = therapistBookings.filter(isCompletedBooking);
    const totalSessions = therapistBookings.length;
    const pendingSessions = therapistBookings.filter((b: any) =>
      String(b.status).toLowerCase().trim() === 'pending_confirmation'
    ).length;

    const totalRevenue = calculateRevenueForBookings(completedSessions);
    ...
    const thisMonthBookings = getBookingsForMonth(therapistBookings, currentMonth, currentYear);
    const monthlyRevenue = calculateRevenueForBookings(thisMonthBookings);

    const lastMonth = currentMonth === 0 ? 11 : currentMonth - 1;
    const lastMonthYear = currentMonth === 0 ? currentYear - 1 : currentYear;

    const lastMonthBookings = getBookingsForMonth(therapistBookings, lastMonth, lastMonthYear);
    const lastMonthRevenue = calculateRevenueForBookings(lastMonthBookings);
  ...
  const generateTherapistTimeSeriesData = () => {
    ...
    const last6Months = Array.from({ length: 6 }, (_, i) => {
      ...
      const monthBookings = getBookingsForMonth(therapistBookings, date.getMonth(), date.getFullYear());
      const completedThisMonth = monthBookings.filter(isCompletedBooking);
      const revenueThisMonth = calculateRevenueForBookings(completedThisMonth);
